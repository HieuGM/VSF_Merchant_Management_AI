"""Import du lieu da chuan bi vao Postgres (schema teammate: backend/database/models.py).

- --dry-run : chi build row + dem, KHONG dung DB (kiem chung mapping).
- (mac dinh): tao bang neu chua co, TRUNCATE, roi bulk insert.
- --core-only: bo qua menu_items + food_images (2 bang nang), chi 5 bang loi.
...
# thu tu insert (cha truoc con sau) va truncate (dao nguoc)
INSERT_ORDER = ["merchants", "operational_metrics", "merchant_profiles",
...
def main():
    ...
    model_map = {
        "merchants": models.Merchant, "operational_metrics": models.OperationalMetric,
        "merchant_profiles": models.MerchantProfile, "reviews": models.Review,
        "delivery_feedbacks": models.DeliveryFeedback, "menu_items": models.MenuItem,
