...
# Active (trace_id, repo) for the currently-running flow. None outside a run_scope.
_active_run: ContextVar[tuple[str, AgentRunRepository] | None] = ContextVar(
...
def _persist(event_type: str, *, agent_name=None, task_name=None, tool_name=None,
             input_payload=None, output_summary=None, status="ok", error_code=None) -> None:
    active = _active_run.get()
    if active is None:
        return  # event outside a tracked run — ignore
    trace_id, repo = active
