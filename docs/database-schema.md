...
Current schema for the merchant domain. PostgreSQL remains the database; no
PostGIS extension is required. The source of truth is
`backend/database/models.py` plus the Alembic revisions:

- `a1b2c3d4e5f6`: runtime/agent records (preference, interaction, agent runs).
- `b1c2d3e4f5a6`: expand + backfill — added typed relational columns/tables and
  populated them from the legacy `dimensions_json`.
- `c2d3e4f5a6b7`: **applied (head)** — full cutover. Created the four remaining
  child tables, converted types, enforced `NOT NULL`, made
  `overall_score_internal` a generated column, and **dropped the legacy profile
  JSON** (`dimensions_json`, `profile_json`, `schema_version`, profile
  `source_kind`), `merchants.open_hours`, and integer `is_demo_target`.

Both migrations were applied to `merchant_platform` on 2026-07-23. The merchant
profile domain is now fully relational; there is no `dimensions_json` fallback.

The four child tables created by `c2d3e4f5a6b7` are populated from the canonical
import artifact `data/profiles.jsonl` via
`backend/scripts/backfill_relational_child_tables_from_jsonl.py` (idempotent
full rebuild): `merchant_dimension_calculations` (8/merchant),
`merchant_dimension_evidence`, `merchant_complaints`, `market_trending_dishes`.

## Design rules

- Merchant profile facts are typed columns or normalized child rows. The merchant
  profile domain contains **no** `JSON`/`JSONB` columns.
...
- Scores use one normalized scale, `0..1`; all eight score columns are
  CHECK-constrained to that range.
- `overall_score_internal` is a **generated** database column (mean of the eight
  scores) for internal QA only; API and agent payloads never expose it.
- Competitors and `distance_km` are not persisted. Nearby queries calculate
  Haversine distance at query time.
- The tool/DTO layer reconstructs the profile document from relational rows
  (`MerchantProfileRepository`); the dimension `price_competitiveness` is emitted
  as `price_level` to preserve the frozen `get_merchant_profile` contract.
- JSONB remains only in runtime/user tables (evolving session state).
