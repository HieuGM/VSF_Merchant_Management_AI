...
- **Catalog:** `core/constraint_catalog.py` — `ConstraintDef(scope, terms, kind avoid|want)` with CANONICAL (diacritic-preserving) terms. Currently seafood (allergy) + chay (diet). Extensible.
- **Collision-safe matcher:** `scope_present(cdef, text_raw, text_folded)` — the single matching primitive used by loader + enforcer. Vietnamese folds tones away, merging crab≈của, mud-crab≈chair, clam≈sauce, vegetarian≈run, and substrings ốc⊂ngọc/tóc. So: multi-word terms → phrase match; single-word terms → whole-TOKEN folded match (kills substring hits sốt⊃sò, ngọc⊃ốc); the handful of TRUE homophones (`cua`, `ghe`, `muc`, `so`, `chay`) ALSO require the canonical diacritic form in the raw text (cua≠của, ghẹ≠ghế, chay≠chạy). This is why "ăn cơm sốt" no longer false-trips seafood and "chạy bộ" no longer imposes vegetarian.
- **Loader:** `services/active_constraints_loader.py` — `build_active_constraints(profile, prior_turns, query) → ActiveConstraints`. ONE pass over all 3 layers + the current message → `{hard, soft}` with provenance. Recall guards: ASK≠declare (`_QUESTION_RE`), negation/recovery suppress (`không ăn chay`, `không còn dị ứng`), diet-break scanned over prior USER turns (not just the current query), the 8-turn window counts USER turns (agent turns no longer halve it), and an in-text durable marker (`từ giờ`) now actually marks the constraint durable.
- **Enforcer:** `services/active_constraints_enforcer.py` — `apply_constraints` (post-search hard-filter, L1 cuisine/name + dish-level L2 partial-overlap: keep a mixed merchant with ≥1 safe dish), `hard_filter_l1` (DB-result level, no dishes loaded), `query_requests_restriction` (confirm-gate when the user explicitly requests an allergen — collision-safe, so "ăn cơm sốt" doesn't false-fire). All matching routes through `scope_present`.
- **Propagation:** `constraints_scope` ContextVar in `core/profile_context.py` (mirrors `profile_scope`) — `should_hard_filter` reads it at the merchant loop. The L1 filter is **always-on** (hoisted out of `ranking_on = cfg.enabled and profile is not None`), so allergies fire for anonymous users and when ranking is disabled (safety > taste).
- **Defense-in-depth:** L1 DB pre-filter (`should_hard_filter`, always-on) → L2 post-search filter (`apply_constraints`, runs BEFORE the top-K slice in the stream path so the explanation never names a dropped merchant) → L3 prompt block (`active_constraints_block` in `_build_explanation_messages`).
...
### Episodic Memory — liked merchants (phase-05)

The 3rd memory layer: WHICH merchants a user engaged with (vs. the constraint layer's WHAT they avoid/want). Powers "gợi ý giống quán tôi từng thích" (memory.txt Test 5/6). Explicit only (heart on the card) — explicit > implicit; decay/skip-capture deferred (the `interaction_events` log is the substrate).

- **Table:** `user_liked_merchants` (`user_id` FK→user_profiles CASCADE, `merchant_id` FK→merchants CASCADE, `liked_at`, PK(user,merchant) = idempotent toggle). Migration `d3e4f5a6b7c8`.
- **Repo:** `repositories/user_liked_merchant_repository.py` — `like` (ON CONFLICT DO NOTHING), `unlike`, `is_liked`, `liked_ids`, `list_details` (id+name+cuisine).
- **Service:** `services/liked_merchant_service.py` — toggles + appends an `interaction_events` audit row (`merchant_liked`/`merchant_unliked`); ensures the `user_profiles` parent row exists (flush) so a first-time user can like before saving any taste.
- **Profile enrichment:** `UserProfilePublic` gains `liked_merchant_ids` + `liked_merchant_cuisines` (derived, not stored), populated by `get_by_id` via one `list_details` query. So the crew's `get_user_profile` tool exposes them → recall + ranking.
- **Ranking** (`profile_ranking.profile_score`): a directly-liked merchant gets `+w_liked_merchant`; and the liked-cuisine branch uses the UNION `liked_cuisines ∪ liked_merchant_cuisines`, so a merchant sharing cuisine with a liked one rises (the "gợi ý giống" effect). Likes are SOFT (boost), never a hard drop.
- **Routes** (`/api/v1/users/{id}/liked-merchants`): `GET` (list), `POST {merchant_id}` (like), `DELETE /{merchant_id}` (unlike) — writes IDOR-guarded (`require_dev_only`).
- **FE:** `LikedMerchantsProvider` (CustomerHome) loads once; `RestaurantCard` heart toggles optimistically + syncs, persists across reload.
