...
├── synthetic/
│   ├── operational.jsonl           # ops metrics + delivery stats + segments (procedural, 1.625); field `scenario` cho 5 quán yếu chủ đích
│   ├── hero_set.json               # 18 hero merchant + lý do chọn (`why`) + `scenario` (5 quán demo)
...
| price_level | giá vs median CÙNG cuisine (peer group); score = độ cạnh tranh giá (ngang/rẻ hơn peers→1.0, chỉ ĐẮT hơn peers mới giảm điểm) | peer_median_price, price_ratio_vs_peers |
...
scripts/synth/scenarios.py               # config 5 kịch bản quán yếu + gán merchant (dùng chung)
