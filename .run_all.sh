...
python -c "import yaml; d=yaml.safe_load(open('backend/agents/customer/config/tasks.yaml',encoding='utf-8')); s=d['search_task']['description']+d['explanation_task']['description']; assert '{cross_conv_context}' in s; print('YAML_OK')"
echo "=== full unit suite ==="
PYTHONPATH=backend python -m pytest backend/tests/unit/ -q -p no:warnings 2>&1 | tail -5
